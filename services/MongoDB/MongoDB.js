 ...
 */
  getHistoricalDbByDate(date = new Date(new Date().toLocaleString('es-CO', { timeZone: 'America/Bogota' })), monthsToAdd = 0) {
    if (monthsToAdd != 0) {
      date.setMonth(date.getMonth() - monthsToAdd);
    }
    if (!date) {
      date = new Date(new Date().toLocaleString('es-CO', { timeZone: 'America/Bogota' }));
    }
  ...
  */
   ...
   */
  generateHistoricDbID(createUtcMillis) {
